...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        auto workRace1 = [this](){return concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());};
        auto workRace2 = [this](){return concurRace2->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());};

        QtConcurrent::run(workRace1).then(workRace2);
        // == ! Тут должен быть код ДЗ ==
