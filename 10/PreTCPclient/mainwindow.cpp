...
{
    ui->tb_result->append(QString("Статус сервера: \nпринято байт: %1").arg(stat.incBytes));
    ...
    ui->tb_result->append(QString("Время работы сервера секунд: %1").arg(stat.workTime));
...
void MainWindow::on_pb_connect_clicked()
{
    if (ui->pb_connect->text() == "Подключиться")
    {
        ...
        QString ip = ui->spB_ip4->text() + "." + ui->spB_ip3->text() + "." + ui->spB_ip2->text() + "." + ui->spB_ip1->text();
